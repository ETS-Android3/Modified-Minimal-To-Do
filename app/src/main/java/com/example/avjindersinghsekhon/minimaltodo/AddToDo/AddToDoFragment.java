...
public class AddToDoFragment extends AppDefaultFragment implements DatePickerDialog.OnDateSetListener, TimePickerDialog.OnTimeSetListener {
    ...
    private final boolean setDateButtonClickedOnce = false;
    private final boolean setTimeButtonClickedOnce = false;
    private LinearLayout mContainerLayout;
    private String theme;
    private LinearLayout mLinearLayout;

    //newspinner
    private String mUserEnteredTopic;
    //R.edit Adding variabel for spinner in fragment_add_to_do.xml
    private Spinner mTodoSpinner;
    private String mTodoTopicSpinner;
...
//        setContentView(R.layout.new_to_do_layout);
        ...
        if (theme.equals(MainFragment.LIGHTTHEME)) {
            ...
            Log.d("OskarSchindler", "Dark Theme");
        ...
        //new spinner
        mUserEnteredTopic= mUserToDoItem.getToDoTopic();
...
//        }
        ...
        topicsAgendaTextView = (TextView) view.findViewById(R.id.textView);
        descEditText = (EditText) view.findViewById(R.id.userToDoDescription);
        mLinearLayout = (LinearLayout) view.findViewById(R.id.addToDoParentLinearLayout);
        if (theme.equals(MainFragment.DARKTHEME)) {
            ...
            topicsAgendaTextView.setTextColor(Color.WHITE);
            descEditText.setTextColor(Color.WHITE);
        ...
        mTodoSpinner = (Spinner) view.findViewById(R.id.userToDoTopicSpinner);
        // mTodoTopicSpinner = mTodoSpinner.getSelectedItem().toString();
...
//        mLastSeenTextView = (TextView)findViewById(R.id.toDoLastEditedTextView);
        ...
        //R.edit Adding adapter for spinner in fragment_add_to_do.xml
        ArrayAdapter<CharSequence> ToDoadapter= ArrayAdapter.createFromResource(getContext(),
                R.array.Topic,
                android.R.layout.simple_spinner_item);
        ToDoadapter.setDropDownViewResource(android.R.layout.simple_spinner_item);

        mTodoSpinner.setAdapter(ToDoadapter);
...
//            mUserReminderDate = new Date();
            ...
            String timeString;
            if (time24) {
                timeString = formatDate("HH:mm", mUserReminderDate);
            } else {
...
//        setDateAndTimeEditText();
    ...
    public void setReminderTextView() {
        if (mUserReminderDate != null) {
            ...
            if (DateFormat.is24HourFormat(getContext())) {
                timeString = formatDate("HH:mm", date);
            } else {
    ...
    public void makeResult(int result) {
        ...
        if (mUserEnteredText.length() > 0) {
            ...
            mUserToDoItem.setToDoTopic(mTodoSpinner.getSelectedItem().toString()) ;
            Log.d(TAG, "topic:" + mTodoSpinner.getSelectedItem().toString());
        } else {
